...
let procs_list =
  let procs_list_core = [type_queries ; numeric_ops; misc_ops; pair_procs; apply_proc] in
  if CompConfig.debug then
    let debug_procs_list = [
      make_routine "display" "
mov r8, PARAMS_COUNT
mov rcx, 0
.loop:
  .loop_check_cond:
